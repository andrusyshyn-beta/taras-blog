...
function translateDirectory(dir) {
    ...
    for(const file of files) {
        ...
        if (fs.statSync(fullPath).isDirectory()) {
            ...
            for (const [ua, en] of Object.entries(replacements)) {
                content = content.split(ua).join(en);
            }
...
translateDirectory('app/en');
translateDirectory('components/en');

const ukLayout = 'app/(uk)/layout.js';
if (fs.existsSync(ukLayout)) {
    let content = fs.readFileSync(ukLayout, 'utf8');
    content = content.replace("'./globals.css'", "'../globals.css'");
    fs.writeFileSync(ukLayout, content, 'utf8');
    console.log("Fixed uk layout");
}
