...
// Direct DOM toggle — bypasses React state entirely
function toggleMenu() {
  const overlay = document.getElementById('ua-mobile-overlay');
  const burger = document.getElementById('ua-burger-btn');
...
export default function Header() {
  ...
  return (
    <>
        ...
        <div className={`container ${styles.inner}`}>
          ...
          <div className={styles.actions}>
            ...
            <div className={styles.langSwitch}>
              <Link href={pathname || '/'} className={`${styles.langBtn} ${styles.active}`}>UA</Link>
              <Link href={`/en${pathname === '/' ? '' : (pathname || '')}`} className={styles.langBtn}>EN</Link>
            </div>

            <button onClick={toggleTheme} className={styles.themeToggle} type="button" aria-label="Тема">
              {theme === 'dark' ? '☀' : '☾'}
            </button>
          ...
          {/* Burger — uses direct DOM toggle, NOT React state */}
          <button
            id="ua-burger-btn"
            type="button"
            data-open="false"
      ...
      {/* 
        Overlay pre-rendered in DOM, hidden via display:none.
        Toggle is via direct DOM style change — no React re-render needed.
      */}
      <div
        id="ua-mobile-overlay"
        style={{
          display: 'none',
          position: 'fixed',
